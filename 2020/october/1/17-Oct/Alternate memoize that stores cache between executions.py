...
class Memorize(object):
    ...
    the current canche is deleted and new cache is created
    (in case the behavior of the function has changed).
    '''
    def __init__(self,func):
        self.func = func
        self.set_parent_file() #sets self.parent_filepath and self.parent_filelname
        self.__name__ = self.func.__name__
        self.set_cache_filename()
        if self.cache_exists():
            self.read cache() #sets self.timestamp and self.cache
    ...
    def set_cache_filename(self):
        ...
        version f the "file_function.cache"
        """
    ...
    def __get__(self,obj,objtype):
        ...
        return functools.patial(self.__call__,obj)
def _slugify(value):
    """
    Normalizes string, converts to lowercase , removes
    non-alpha cahracters, and converts spaces to
    hyphens.
    """
